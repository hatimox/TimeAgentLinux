...
impl<T: Tray + 'static> InnerState<T> {
    ...
    fn update_menu(&self) {
        ...
        if layout_updated {
            // The layout changed. Bump the revision and emit LayoutUpdated so the
            // host refetches — that alone is enough. NOTE: upstream also shifts
            // every item id by `item_id_offset` here "to invalidate all items",
            // but that breaks the GNOME/Ubuntu AppIndicator extension: it fetches
            // the layout and item properties in separate D-Bus calls, and if the
            // ids change in between the labels never bind and the menu renders
            // empty. Keeping ids stable (index-based) fixes that. (local patch)
